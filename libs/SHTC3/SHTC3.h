...
{
  public:
    SHTC3(arduino_shtc3_wire_t &w = Wire) : _wire(w), _t(0), _h(0)
    {
    }
    ...
    inline bool sleep()
    {
        return twiCommand(SHTC3_SLEEP);
    }
    inline bool wakeup()
    {
        bool b = twiCommand(SHTC3_WAKEUP);
        delayMicroseconds(SHTC3_RESET_DELAY_US);
        return b;
    }
    inline bool reset()
    {
        bool b = twiCommand(SHTC3_RESET);
        delayMicroseconds(SHTC3_RESET_DELAY_US);
        return b;
    }
  ...
  private:
    ...
    uint16_t _h;
    // Private methods
    inline bool checkCRC(const uint8_t *data)
    {
